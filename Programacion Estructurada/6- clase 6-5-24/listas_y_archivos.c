...
int main(){
    ...
    archivo=fopen("datosx.txt","r");    //funciones fopen para abrir un archivo, 0--> OK
    fclose(archivo);//funcion fclose para cerrar un archivo
    //un error comun puede ser que no abra el archivo porque no exista.
    archivo= fopen("datos.txt", "w");//datos es el txt que devuelve y devuelve un puntero que almacena en a y la w es el modo(w-write, r-read, a-abre pero no borra)
